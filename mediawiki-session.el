...
- \='symmetric: Use symmetric encryption (password prompt) \='recipient:
- Encrypt to specific recipient (set mediawiki-session-gpg-recipient)"
...
Implements ERROR-CODE recovery."
...
Implements automatic ERROR-CODE recovery with retry logic.
Optionally, limit to RETRY-COUNT."
...
(defun mediawiki-session-store-credentials (sitename _credentials)
...
If FORCE is non-nil, perform server-side validation regardless of cache."
  (let ((session (mediawiki-get-session sitename))
    ...
    (if (not session)
      ...
      (if (not (and (mediawiki-session-login-time session)
...
(defun mediawiki-session-validate-all ()
  ...
  (let ((validated-count 0)
    ...
    (maphash (lambda (sitename _session)
               (condition-case err
                 (cond
...
(defun mediawiki-session-determine-encryption-method ()
  "Determine the best encryption method to use based on configuration."
  (cond
...
(defun mediawiki-session-migrate-v1-to-v2 (storage-data)
  "Migrate session STORAGE-DATA from version 1 to version 2."
...
(defun mediawiki-session-migrate-unknown-version (storage-data)
  "Handle migration of STORAGE-DATA from unknown version with best effort."
...
OPERATION-TYPE is the operation type that caused this."
  (require 'mediawiki-api)
...
(defun mediawiki-session-cleanup-token-cache (sitename)
  "Clean up token cache for SITENAME by removing expired tokens.
...
(defun mediawiki-session-get-state-summary ()
  ...
  (let ((summary (make-hash-table :test 'eq))
    ...
    (maphash (lambda (_sitename state)
