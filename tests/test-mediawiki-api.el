...
;;; Test New API Functions (Added for Issue #39)

(ert-deftest test-mediawiki-api-get-page-history ()
  "Test mediawiki-api-get-page-history with mocked API call."
             ...
             (lambda (site action args)
               '((query . ((pages . ((\1 . ((pageid . 1)
                                            (revisions . (((revid . 101)
                                                           ...
                                                           (size . 100)
                                                           (minor . t))))))))))))))
    (let ((result (mediawiki-api-get-page-history "TestSite" "Test Page")))
      (should (listp result))
      (should (= 1 (length result)))
      (let ((rev (car result)))
        (should (= 101 (alist-get 'revid rev)))
        (should (= 100 (alist-get 'parentid rev)))
        (should (string= "2026-01-01T00:00:00Z" (alist-get 'timestamp rev)))
        (should (string= "User1" (alist-get 'user rev)))
        (should (string= "test edit" (alist-get 'comment rev)))
        (should (= 100 (alist-get 'size rev)))
        (should (alist-get 'minor rev))))))

(ert-deftest test-mediawiki-api-get-revision-content ()
  "Test mediawiki-api-get-revision-content with mocked API call."
  (cl-letf (((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((query . ((pages . ((\1 . ((pageid . 1)
                                            (revisions . (((revid . 101)
                                                           (slots . ((main . ((* . "Page content"))))))))))))))))))
    (let ((result (mediawiki-api-get-revision-content "TestSite" "Test Page" 101)))
      (should (stringp result))
      (should (string= "Page content" result)))))

(ert-deftest test-mediawiki-api-get-latest-revision-content ()
  "Test mediawiki-api-get-latest-revision-content with mocked API call."
  (cl-letf (((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((query . ((pages . ((\1 . ((pageid . 1)
                                            (revisions . (((revid . 101)
                                                           (slots . ((main . ((* . "Page content"))))))))))))))))))
    (let ((result (mediawiki-api-get-latest-revision-content "TestSite" "Test Page")))
      (should (consp result))
      (should (= 101 (car result)))
      (should (string= "Page content" (cdr result))))))

(ert-deftest test-mediawiki-api-compare-revisions ()
  "Test mediawiki-api-compare-revisions with mocked API call."
  (cl-letf (((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((compare . ((body . ((* . "<tr><td>diff html</td></tr>")))))))))
    (let ((result (mediawiki-api-compare-revisions "TestSite" 100 101)))
      (should (stringp result))
      (should (string= "<tr><td>diff html</td></tr>" result)))))

(ert-deftest test-mediawiki-api-get-watchlist ()
  "Test mediawiki-api-get-watchlist with mocked API call."
  (cl-letf (((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((query . ((watchlist . (((title . "Page A")
                                          (revid . 101)
                                          (timestamp . "2026-01-01T00:00:00Z")
                                          (user . "User1")
                                          (comment . "edit")
                                          (oldlen . 50)
                                          (newlen . 100))))))))))
    (let ((result (mediawiki-api-get-watchlist "TestSite" 1 1)))
      (should (listp result))
      (should (= 1 (length result)))
      (let ((entry (car result)))
        (should (string= "Page A" (alist-get 'title entry)))
        (should (= 101 (alist-get 'revid entry)))
        (should (string= "User1" (alist-get 'user entry)))))))

(ert-deftest test-mediawiki-api-set-watch ()
  "Test mediawiki-api-set-watch with mocked API calls."
  (cl-letf (((symbol-function 'mediawiki-site-get-token)
             (lambda (site type) "testtoken+\\"))
            ((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((watch . ((title . "Page A") (watched . "")))))))
    (let ((result (mediawiki-api-set-watch "TestSite" "Page A" nil)))
      (should (listp result))
      (should (string= "Page A" (alist-get 'title result))))))

(ert-deftest test-mediawiki-api-mark-page-seen ()
  "Test mediawiki-api-mark-page-seen with mocked API calls."
  (cl-letf (((symbol-function 'mediawiki-site-get-token)
             (lambda (site type) "testtoken+\\"))
            ((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((setnotificationtimestamp . ((notificationtimestamp . "")))))))
    (let ((result (mediawiki-api-mark-page-seen "TestSite" "Page A")))
      (should (listp result))
      (should (string= "" (alist-get 'notificationtimestamp result))))))

(ert-deftest test-mediawiki-api-get-user-contributions ()
  "Test mediawiki-api-get-user-contributions with mocked API call."
  (let ((old-fn (symbol-function 'mediawiki-api-call)))
    (unwind-protect
        (progn
          (fset 'mediawiki-api-call
                (lambda (site action args)
                  '((query . ((usercontribs . (((revid . 101)
                                               (title . "Page A")
                                               (timestamp . "2026-01-01T00:00:00Z")
                                               (user . "User1")
                                               (comment . "edit")
                                               (sizediff . 50)))))))))
          (let ((result (mediawiki-api-get-user-contributions "TestSite" "User1")))
            (should (listp result))
            (should (= 1 (length result)))
            (let ((contrib (car result)))
              (should (= 101 (alist-get 'revid contrib)))
              (should (string= "Page A" (alist-get 'title contrib)))
              (should (string= "User1" (alist-get 'user contrib))))))
      (fset 'mediawiki-api-call old-fn))))

(ert-deftest test-mediawiki-api-thank-revision ()
  "Test mediawiki-api-thank-revision with mocked API calls."
  (cl-letf (((symbol-function 'mediawiki-site-get-token)
             (lambda (site type) "testtoken+\\"))
            ((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((result . "Success")))))
    (let ((result (mediawiki-api-thank-revision "TestSite" 101)))
      (should (eq result t)))))

(ert-deftest test-mediawiki-api-preview ()
  "Test mediawiki-api-preview with mocked API call."
  (cl-letf (((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((parse . ((text . ((* . "<p>Preview HTML</p>")))))))))
    (let ((result (mediawiki-api-preview "TestSite" "Test Page" "'''bold''' text")))
      (should (stringp result))
      (should (string= "<p>Preview HTML</p>" result)))))

(ert-deftest test-mediawiki-api-undo-revision ()
  "Test mediawiki-api-undo-revision with mocked API calls."
  (cl-letf (((symbol-function 'mediawiki-site-get-token)
             (lambda (site type) "testtoken+\\"))
            ((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((edit . ((result . "Success")))))))
    (let ((result (mediawiki-api-undo-revision "TestSite" "Test Page" 101 "undo test edit")))
      (should (eq result t)))))

(ert-deftest test-mediawiki-api-restore-revision ()
  "Test mediawiki-api-restore-revision with mocked API calls.
This function chains get-revision-content, site-get-token, and a second api-call."
  (cl-letf (((symbol-function 'mediawiki-api-get-revision-content)
             (lambda (site title revid) "Restored page content"))
            ((symbol-function 'mediawiki-site-get-token)
             (lambda (site type) "testtoken+\\"))
            ((symbol-function 'mediawiki-api-call)
             (lambda (site action args)
               '((edit . ((result . "Success")))))))
    (let ((result (mediawiki-api-restore-revision "TestSite" "Test Page" 100)))
      (should (eq result t)))))

(ert-deftest test-mediawiki-api-call-async ()
  "Test mediawiki-api-call-async calls callback with non-nil on success."
  (cl-letf (((symbol-function 'mediawiki-make-api-url)
             (lambda (&optional site) "https://example.com/w/api.php"))
            ((symbol-function 'mm-url-encode-www-form-urlencoded)
             (lambda (params) "title=Test&format=json&action=edit"))
            ((symbol-function 'url-retrieve)
             (lambda (url cb &rest args)
               (funcall cb nil)
               nil)))
    (let ((callback-arg 'unset))
      (mediawiki-api-call-async "TestSite" "edit" '((title . "Test"))
        (lambda (success)
          (setq callback-arg success)))
